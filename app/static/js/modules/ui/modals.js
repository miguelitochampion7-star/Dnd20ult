...
export function showToast(message, type = 'success') {
    ...
    toast.className = `min-w-[250px] bg-black/90 backdrop-blur border ${borderColor} text-white px-4 py-3 rounded shadow-lg mb-3 flex items-center gap-3 transform translate-x-full transition-all duration-300`;
    toast.innerHTML = `
        <span class="text-xl">${icon}</span>
        <div class="flex-1">
            <p class="font-bold text-xs ${textColor} uppercase tracking-wider">${type === 'error' ? 'Error' : 'Éxito'}</p>
